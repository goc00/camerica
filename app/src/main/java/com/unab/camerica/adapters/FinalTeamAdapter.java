...
public class FinalTeamAdapter extends ArrayAdapter<Equipo> {

    public FinalTeamAdapter(Context context, ArrayList<Equipo> data) {
        super(context, R.layout.activity_final_predictor, data);
    }

    public View getView(final int position, View convertView, ViewGroup parent) {
        Equipo actual = this.getItem(position);
        View item = convertView;

        /*PartidoAdapter.PartidoHolder holder;

        if(item == null) {
            LayoutInflater inflater = LayoutInflater.from(getContext());
            item = inflater.inflate(R.layout.partido, null);
            holder = new PartidoAdapter.PartidoHolder();
            ...
            holder = (PartidoAdapter.PartidoHolder)item.getTag();
        }
